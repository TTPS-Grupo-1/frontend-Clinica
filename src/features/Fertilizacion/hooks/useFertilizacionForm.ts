...
export const useFertilizacionForm = (currentUser: any, ovocitos: any[], onClose: () => void) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const fertilizacionResponse = await axios.post(
        'http://localhost:8000/api/fertilizacion/',
        payload,
        { headers }
      );
      ...
      if (payload.resultado === 'exitosa' && fertilizacionResponse.data.success) {
        const ovocitoUtilizado = ovocitos.find((o) => (o.id_ovocito ?? o.id) === payload.ovocito);
        ...
        if (ovocitoUtilizado) {
          ...
          console.log(
            `✅ Embrión ${identificadorEmbrion} creado para fertilización ${fertilizacionId}`
          );
        }
      }
  ...
  const ejecutarProceso = async (
    ...
    try {
      ...
      const basePayload: any = {
        ...
        banco_semen_id: tieneBancoSeleccionado
          ? (bancoSemenSeleccionado.id ?? bancoSemenSeleccionado.identificador ?? null)
          : null,
      ...
      const targetOvocitos =
        ovocitoSeleccionado !== null
      ...
      for (const ov of targetOvocitos) {
        ...
        const esCrio =
          (ov as any).tipo_estado &&
          (ov as any).tipo_estado.toString().toLowerCase() === 'criopreservado';
