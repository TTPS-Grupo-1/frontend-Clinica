...
export default function SacarTurno() {
    ...
    useEffect(() => {
        const fetchMedicos = async () => {
            ...
            if (modoReasignar && medicoReasignar) {
                ...
                toast.error("No se encontró el médico localmente; si no aparece, revisá la lista de médicos.");
                }
            } else {
    ...
    const handleReasignarTurno = async () => {
        ...
        try {
            ...
            const cancelarRes = await fetch(
                `${PROXY_CANCELAR_URL}?id_turno=${turnoIdReasignar}`,
                { method: "PATCH" }
            );
        ...
        <div className="min-h-screen bg-gray-100 p-6 flex flex-col items-center pt-[80px]">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-md w-full max-w-2xl space-y-6 text-gray-800"> 
                ...
                {modoReasignar && fechaHora && (
                    <div className="p-3 bg-blue-50 rounded-lg border border-blue-200 text-center text-blue-700 text-sm">
                        <p><strong>Reasignando turno:</strong></p>
                        <p>
                        ...
                        <p><strong>Puede seleccionar ±1 día en el mismo horario y con el mismo médico. Si no hay disponibles, no podrá modificarlo.</strong></p>
                    </div>
                )}
                ...
                <Calendario
                    ...
                    disabled={(date) => {
                        ...
                        const inicioTry = new Date(date.getFullYear(), date.getMonth(), date.getDate()).getTime();
