            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
              {ordenes.map((orden: any, index: number) => (
                <div key={index} className="rounded-lg border border-blue-200 bg-blue-50 p-3">
                  ...
                  <p className="text-xs text-blue-600">{formatDateShort(orden.fecha_creacion)}</p>
                </div>
              ))}
            </div>
          </div>
                    ...
                    {resultados.map((resultado: any, index: number) => (
                      <div key={index} className="rounded-lg bg-gray-50 p-3">
                        <div className="mb-1 flex items-center justify-between">
                          ...
                          {resultado.valor && <CheckCircle className="h-4 w-4 text-green-500" />}
                        </div>
                        <p className="text-sm text-gray-600">{resultado.valor || 'Pendiente'}</p>
                        {resultado.tipo_estudio && (
                          <span className="mt-1 inline-block rounded bg-gray-200 px-2 py-1 text-xs text-gray-600">
                            {resultado.tipo_estudio}
                          </span>
                        )}
                      </div>
        ...
        Object.keys(groupedEstudios).length > 0 && (
          <div className="mt-6 space-y-6">
            ...
            {Object.entries(groupedEstudios).map(([tipo, estudiosTipo]) => (
              <div key={tipo}>
                <div
                  className={`mb-3 inline-flex items-center gap-2 rounded-lg border px-3 py-1 text-sm font-medium ${getTypeColor(tipo)}`}
                >
