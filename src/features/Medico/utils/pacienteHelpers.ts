 ...
 */
export async function fetchPacientesForMedico(
  medicoId: number,
  ...
  const tratamientosConMedico = tratamientos.filter((tr: any) => {
    ...
    const m =
      tr?.medico ?? tr?.medico_id ?? tr?.medico_tratante ?? tr?.medico_tratante_id ?? tr?.medicoId;
  ...
  const tratamientosFiltrados =
    tratamientosConMedico.length > 0 ? tratamientosConMedico : tratamientos;
  ...
  for (const t of turnos) {
    const pid =
      typeof t.paciente === 'object'
        ? (t.paciente.id ?? t.paciente_id)
        : (t.paciente ?? t.paciente_id);
    if (!pid) continue;
 ...
 */
export async function fetchPacienteIfHasTratamiento(
  pacienteId: number,
  headers: Record<string, string> = {}
): Promise<PacienteMinimal | null> {
  try {
    ...
    if (tratamientoExistente) {
      return {
        id: pacienteId,
        first_name: pacienteRes.data.first_name,
        last_name: pacienteRes.data.last_name,
 ...
 */
export async function fetchPacientesByName(
  query: string,
  headers: Record<string, string> = {}
): Promise<PacienteMinimal[]> {
    ...
    await Promise.all(
      list.map(async (p: any) => {
        ...
        try {
          ...
          const tiene = Boolean(
            tRes.data &&
              (Array.isArray(tRes.data) ? tRes.data.length > 0 : Object.keys(tRes.data).length > 0)
          );
          if (tiene) {
...
export async function marcarTurnoAtendido(id: number): Promise<boolean> {
  try {
    ...
    console.error(
      'Error marcando turno como atendido:',
