...
export async function obtenerDeudaPaciente(id_paciente: number, numero_grupo: number) {
  ...
  try {
    ...
    throw new Error(
      ...
      "Error consultando deuda del paciente"
    );
  }
}

export async function registrarPagoSupabase(id_grupo: number, obra_social_pagada: boolean, paciente_pagado: boolean, id_paciente: number) {
