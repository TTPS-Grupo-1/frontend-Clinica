...
export default function MonitoreoView({ monitoreo }: MonitoreoViewProps) {
  ...
  const getEstadoColor = (estado: string) => {
    ...
    if (
      estadoLower.includes('normal') ||
      estadoLower.includes('óptimo') ||
      estadoLower.includes('bien')
    ) {
            ...
            <div className="flex gap-4">
              ...
              <div className="flex-1 rounded-lg border border-gray-200 bg-gray-50 p-4">
                <div className="mb-3 flex items-start justify-between">
                  <div>
                    <h3 className="font-medium text-gray-800">Día {index + 1} del tratamiento</h3>
                ...
                <div className="mt-4 space-y-2">
                  {registro.estado && (
                    <div
                      className={`inline-flex items-center gap-1 rounded px-2 py-1 text-xs font-medium ${getEstadoColor(registro.estado)}`}
                    >
      ...
      <div className="mt-6 rounded-lg bg-cyan-50 p-4">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm md:grid-cols-4">
          ...
          <div className="text-center">
            <div className="font-bold text-cyan-700">
              {formatDate(
                monitoreo[monitoreo.length - 1]?.fecha ||
                  monitoreo[monitoreo.length - 1]?.fecha_monitoreo
              )}
            </div>
