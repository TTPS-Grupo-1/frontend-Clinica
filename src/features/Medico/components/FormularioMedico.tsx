...
import type { FormMedicoProps } from "../../../interfaces/Medico";
...
export default function FormularioMedico({ onRegistrar, initialValues, titulo, botonTexto }: FormMedicoProps & { titulo?: string, botonTexto?: string }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTelefono("");
      ...
      // Paso 2: setear estados iniciales si initialValues existe
      useEffect(() => {
        if (typeof initialValues !== "undefined") {
          setNombre(initialValues.nombre ?? "");
          setApellido(initialValues.apellido ?? "");
          setDni(initialValues.dni !== undefined ? String(initialValues.dni) : "");
          setEmail(initialValues.email ?? "");
          setTelefono(initialValues.telefono !== undefined ? String(initialValues.telefono) : "");
        }
      }, [initialValues]);
  return (
    <div className="max-w-md mx-auto mt-24 p-6 bg-gray-100 rounded shadow border border-gray-300">
      <h1 className="text-2xl font-bold mb-6 text-center text-gray-800">{titulo}</h1>
